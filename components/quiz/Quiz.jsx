...
const Quiz = () => {
    ...
    const fetchQuizData = async () => {
        if (selectedNumofQuestion && selectedSubject) {
            const question = await fetchQuizForUser(selectedNumofQuestion, selectedSubject)
    ...
    const handleAnswerChange = (questionId, answer) => {
        setSelectedAnswers((prevAnswers) => {
            const existingAnswerIndex = prevAnswers.find((answerObj) => answerObj.id === questionId)
            ...
            if (existingAnswerIndex !== -1) {
                ...
                const newAnswers = { id: questionId, answer: selectedAnswer }
